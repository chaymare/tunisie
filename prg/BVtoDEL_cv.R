...
BV2$txparticipation <- round((BV.df$Votants/BV.df$Inscrits)*100,2)
BV2$txexprimes <- round((BV.df$V_exprm/BV.df$Votants)*100,2)
BV2$txnuls <- round((BV.df$V_nuls/BV.df$Votants)*100,2)
BV2$txblancs <- round((BV.df$V_blancs/BV.df$Votants)*100,2)
colnames(BV2)
...
for (i in 20:46){
  ...
  colnames(BV2)[j] <- paste("TX",colnames(BV.df)[i],sep="")
}
head(BV2)

# Aggrégation
cv <- function(x){sd(x)/mean(x)}
CvByDeleg.df <- aggregate(BV2[,c(3:32)], by = list(BV2$del_id), FUN=cv)
head(CvByDeleg.df)
